...
public class HTTP303ComplianceTest extends GWIntegrationTest {
    private HttpServerOperationBuilderContext emulator;
    private String host = "127.0.0.1";
    private int port = 9090;
    private File responsePayload;
    private String headerLocation = "http://different.uri";

    @BeforeClass
    public void setup() throws Exception {
        responsePayload = new File(getClass().getClassLoader()
                .getResource("test-payloads" + File.separator + "303responsepage.html").toURI());
    ...
    private HttpServerOperationBuilderContext startHttpEmulator() {
        return Emulator.getHttpEmulator().server().given(configure().host("127.0.0.1").port(6065).context("/users"))
                ...
                .when(request()
                        ...
                        .withPath("/user1"))
                .then(response()
                        .withStatusCode(HttpResponseStatus.SEE_OTHER)
    ...
    @Test
    public void test303GETRequest() throws Exception {
        ...
        Assert.assertEquals(response.getReceivedResponse().getStatus(), HttpResponseStatus.SEE_OTHER,
    ...
    @Test
    public void test303HEADRequest() throws Exception {
        HttpClientResponseProcessorContext response = Emulator.getHttpEmulator().client()
                .given(HttpClientConfigBuilderContext.configure().host(host).port(port))
    ...
    @Test
    public void test303POSTRequestWithPayload() throws Exception {
        HttpClientResponseProcessorContext response = Emulator.getHttpEmulator().client()
                .given(HttpClientConfigBuilderContext.configure().host(host).port(port))
    ...
    @Test
    public void test303POSTRequestWithoutPayload() throws Exception {
        HttpClientResponseProcessorContext response = Emulator.getHttpEmulator().client()
                .given(HttpClientConfigBuilderContext.configure().host(host).port(port))
    ...
    @Test
    public void test303POSTRequestWithoutPayloadWithContentType() throws Exception {
        HttpClientResponseProcessorContext response = Emulator.getHttpEmulator().client()
                .given(HttpClientConfigBuilderContext.configure().host(host).port(port))
